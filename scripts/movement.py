...
def movement_forward():
	...
	queue_size is an argumane that limits the amount of queued messages if a subscriber is not receiving them fast enough.'''
	...
	while not rospy.is_shutdown():
		status_key = "Moving Husky with velocity:"+" "+str(move_cmd.linear.x)+" "+"m/s"
		rospy.loginfo(status_key)
...
def movement_main():
	'''This is the main function where the GoForward node is declared and initialized, then the movement_forward() code is invoked.
	GoForward then communicates with ROS Master.'''
